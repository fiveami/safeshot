...
def _merge_boxes_iou(boxes, iou_thresh=0.35):
    """Merge overlapping boxes using IoU clustering. Boxes are (x1,y1,x2,y2,score).
    Returns list of merged (x1,y1,x2,y2,score).
    """
    if not boxes:
        return []
    ...
    for i, b in enumerate(boxes):
        ...
        for j in range(i + 1, len(boxes)):
            ...
            # merge if IoU sufficiently high or if one box mostly contains the other
            if iou > iou_thresh or (inter / area2 > 0.7) or (inter / area1 > 0.7):
                group.append(boxes[j])
                taken[j] = True
        # merged extents and score
...
def _apply_clahe_bgr(img_bgr, clip=2.0, tile=(8, 8)):
    """Apply CLAHE (adaptive histogram equalization) to BGR image to improve contrast in varied lighting."""
    try:
...
def _load_dnn_net():
    ...
    modelFile = os.path.join(base, "models", "res10_300x300_ssd_iter_140000.caffemodel")
...
def _get_aggressiveness_params(level: str):
...
def apply_safe_mode(pixmap: QPixmap, diagnostics: bool = False, aggressiveness: str = "medium") -> QPixmap:
    ...
    try:
        ...
        params = _get_aggressiveness_params(aggressiveness)
        if diagnostics:
        ...
        if net is not None:
            ...
            for s in scales:
                ...
                resized_enh = _apply_clahe_bgr(resized, clip=params.get("clahe_clip", 2.0), tile=(8, 8))
            ...
            try:
                ...
                for s in mt_scales:
                    ...
                    try:
                        ...
                        target_bgr = _apply_clahe_bgr(target_bgr, clip=params.get("clahe_clip", 2.0), tile=(8, 8))
        ...
        merged_with_scores = _merge_boxes_iou(face_boxes, iou_thresh=0.35)
...
def _download_dnn_models():
    """Download the required DNN face detector files into the models/ folder.

    Uses raw GitHub URLs for OpenCV model files. Raises on failure.
    """
...
class SnipWindow(QWidget):
    def __init__(self, on_done=None, diagnostics: bool = False, aggressiveness: str = "Medium") -> None:
        super().__init__()
    ...
    def mouseReleaseEvent(self, event: QMouseEvent) -> None:
        if event.button() == Qt.MouseButton.LeftButton and self.begin is not None:
            ...
            if rect.width() > 0 and rect.height() > 0:
                try:
                    # Account for device pixel ratio: map widget coords -> pixmap (physical) coords
                    try:
                        scale = float(self.desktop_pixmap.devicePixelRatioF())
                    except Exception:
                        # Fallback to integer devicePixelRatio if F not available
                    ...
                    # Crop the desktop pixmap to the selected (physical) rectangle
                    cropped = self.desktop_pixmap.copy(src_rect)

                    # >>> SAFE MODE MAGIC HERE <<<
                    safe_pixmap = apply_safe_mode(
                        cropped, diagnostics=self._diagnostics, aggressiveness=self._aggressiveness
                    )

                    # Save to a timestamped file in current directory
                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                    ...
                    # If any error happens during processing, report it but don't crash the app
            ...
            # Close the snip overlay after capture (like Windows snip tool)
            self.close()
    ...
    def closeEvent(self, event):
        # Ensure the callback is invoked at least once when the overlay closes
        if callable(self.on_done) and not self._done_called:
            try:
                self.on_done(None, None)
            finally:
    ...
    # --- Drawing the dimmed background and selection box ---

    def paintEvent(self, event: QPaintEvent) -> None:
        qp = QPainter(self)
        qp.setRenderHint(QPainter.RenderHint.Antialiasing)

        # Draw the captured desktop as the background scaled to window size
        if not self.desktop_pixmap.isNull():
            qp.drawPixmap(self.rect(), self.desktop_pixmap, self.desktop_pixmap.rect())

        # Draw translucent overlay to dim the screen
        overlay_color = QColor(0, 0, 0, 120)
        qp.fillRect(self.rect(), overlay_color)

        # If we have a selection, draw the undimmed selection and border
        if self.begin is not None and self.end is not None:
            sel_rect = QRect(self.begin, self.end).normalized()

            # Compute source rect on the pixmap (physical coords)
            try:
                scale = float(self.desktop_pixmap.devicePixelRatioF())
            except Exception:
                scale = float(self.desktop_pixmap.devicePixelRatio())
            ...
            # Draw the actual (undimmed) area from the desktop pixmap
...
class MainWindow(QWidget):
    def __init__(self) -> None:
        ...
        # Global styling – make it look more like a real tool
        self.setStyleSheet(
            """
            QWidget {
        ...
        # Compact horizontal toolbar (top) with small buttons similar to Windows snipping tool
        ...
        # Thumbnail preview sits beneath the toolbar, no overlap with buttons
    ...
    def start_snip(self):
        # Hide main window while snipping so screen content is unobstructed
        self.hide()

        def on_done(pixmap, filename):
            # Save last and show thumbnail
            ...
            self.show()
            # drop reference to snip window so it can be GC'd
            try:
        ...
        try:
            ...
            # If snip creation fails, restore main window and report
